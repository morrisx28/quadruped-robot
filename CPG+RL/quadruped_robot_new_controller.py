...
import quadruped_robot_new_gait
import log
import gait
...
class QuadrupedRobotController(quadruped_robot_model.QuadrupedRobot):
    ...
    def executeGaitCycle(self, gait, cycle=1):
        gait_count = 0
        self.slowlyPosition(
            lf_cmd=gait.start_point_lf,
            rf_cmd=gait.start_point_rf,
            lh_cmd=gait.start_point_lh,
            rh_cmd=gait.start_point_rh,
        ...
        if gait.bool_has_init_gait:
            while gait_count != gait.number_of_init_point:
                ...
                self.executeIK(
                    lf_cmd, rf_cmd, lh_cmd, rh_cmd, lf_pos, rf_pos, lh_pos, rh_pos
                )
                gait_count+=1
    ...
    def trotForward(self):
        gait = quadruped_robot_new_gait.TrotForwardGait(resolution=6)
        gait.changeDt(0.05)
        self.executeGaitCycle(gait=gait,cycle=30)
...
if __name__ == "__main__":
    ...
    command_dict = {
        ...
        "forward":robot.trotForward,
        "ROS":robot.activateROSMode,
        "reset":robot.resetROS,
    }
